...
If you want the exact steps, I wrote a full setup guide covering everything from macOS user creation to Discord bot configuration to local model tuning. Link at the bottom.
...
Heartbeats, log scanning, drafting background context for morning briefs: local model work. Slower, dumber, and free. Nobody waits on it. Complex reasoning, actual coding work, anything that writes to memory: Claude. The latency and capability difference matters, and so does the quota.
